  ...
  void initState() {
    ...
    ever(controller.routesList, (routes) {
      if (routes.isNotEmpty) {
        // Reset local state when list changes (e.g. from filters)
        setState(() {
          currentRouteIndex = 0;
          selectedRoute = null;
        });

        // Auto-display the first route when data is loaded or changed
  ...
  Future<void> _centerMapOnCurrentLocation() async {
    try {
      final camera = mapbox.CameraOptions(
        center: mapbox.Point(
          coordinates:
              mapbox.Position.fromJson([90.4125, 23.8103]), // Dhaka coordinates
        ),
        zoom: 14.0,
      );
