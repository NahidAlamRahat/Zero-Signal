import 'package:socket_io_client/socket_io_client.dart' as io
    show Socket, DartySocket, OptionBuilder, io;
    ...
    try {
      if (isConnected) {
        ...
        _onceConnected(() {
          appRootSocket?.emit(event, data);
          appLog(
              "✅ Emitted event after connection: $event with data: $data"); // ✅ Added logging
        });
      }
    } catch (e, stackTrace) {
      errorLog(
        "emitEvent ($event) $e $stackTrace",
      );
    }
  }
    ...
    try {
      ...
      appRootSocket = io.io(
        ...
        io.OptionBuilder()
            ...
            .disableAutoConnect()
            .setExtraHeaders(
                {'foo': 'bar', "Authorization": "Bearer ${LocalStorage.token}"})
            .enableReconnection()
            .build(),
      );
